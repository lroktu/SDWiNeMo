...
# Configura fluxos para o sta13 (CoSB) e sta1 e sta2 (CoSC), conectados no AP3
...
if [ $1 == ssid_0 ]; then
	...
	ovs-ofctl add-flow s1 "idle_timeout=180,priority="$pri",dl_type=0x0800,nw_src="$staY",nw_dst="$staX",in_port=9,actions=output:4"
	ovs-ofctl add-flow s1 "idle_timeout=180,priority="$pri",dl_type=0x0800,nw_src="$staX",nw_dst="$staY",in_port=4,actions=output:9"
    ...
    # Fluxos para o Switch
	ovs-ofctl add-flow s1 "idle_timeout=180,priority="$pri",nw_src="$staY",nw_dst="$staX",in_port=8,actions=mod_dl_dst:02:00:00:00:0a:00,mod_nw_dst:"$staXX",output:1"
 ...
 #   ovs-ofctl del-flows s1 "nw_src="$staY",nw_dst="$staX""
    ...
    # Fluxos para o Switch
    ovs-ofctl add-flow s1 "idle_timeout=180,priority="$pri",dl_type=0x0800,nw_src="$staY",nw_dst="$staX",in_port=8,actions=mod_dl_dst:02:00:00:00:0a:00,mod_nw_dst:"$staXX",output:2"
...
#    ovs-ofctl add-flow s1 "idle_timeout=180,priority="$pri",nw_src="$staY",nw_dst="$staX",in_port=8,actions=output:3"
    ...
    # Fluxos para o Switch
    ovs-ofctl add-flow s1 "idle_timeout=180,priority="$pri",nw_src="$staY",nw_dst="$staX",in_port=8,actions=output:4"
    ovs-ofctl add-flow s1 "idle_timeout=180,priority="$pri",nw_src="$staX",nw_dst="$staY",in_port=4,actions=output:8"
